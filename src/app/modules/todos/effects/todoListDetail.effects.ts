...
export class TodoListDetailEffects extends CoreEffects {
  ...
  @Effect()
  public editTodoList$ = this.actions$.pipe(
    ofType(todoListDetailActions.TodoListDetailActionTypes.EditTodoList),
    map((action: todoListDetailActions.EditTodoList) => action.payload),
    exhaustMap(payload => {
      return this.todosService.updateTodoList(payload).pipe(
        map(() => {
          return new todoListDetailActions.EditTodoListSuccess();
        }),
        catchError(err => {
          this.commonErrorHandler(err);
          return of(new todoListDetailActions.EditTodoListFailure());
        }),
      );
    }),
  );
